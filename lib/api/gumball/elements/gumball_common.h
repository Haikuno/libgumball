...
//! Passes a backend-native event to libGumball. Polling backends may ignore it.
...
/*!  \brief Draws all draw-enabled Widgets below the active Root
 *
 *   Widgets are ordered by z-index, hierarchy depth, then Root membership order.
 *   Drawing uses a retained snapshot, so draw-order mutations apply next traversal.
 *   Optionally takes a GUM_Renderer if your backend needs one (defaults to nullptr).
*/
...
//! Adds the second element as a child of the first and updates affected layout state.
#define GUM_add_child(self, child)    ((GUM_add_child)    (GBL_OBJECT(self), GBL_OBJECT(child)))

//! Removes a direct child and updates affected layout state.
...
//! Releases an element and recursively releases its children.
...
//! Sets a property. Setting "parent" also updates libGumball hierarchy state.
#define GUM_setProperty(obj, name, /*value*/...) ((GUM_setProperty)(GBL_OBJECT(obj), name __VA_OPT__(,) __VA_ARGS__))

//! Creates a NULL-terminated list for the inherited construction-time children property.
