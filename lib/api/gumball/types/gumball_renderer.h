...
//! Creates a renderer wrapper when required by the active backend. The passed renderer remains caller-owned and must outlive the wrapper.
GUM_Renderer* GUM_Renderer_create  (void* pRenderer);
//! Destroys the renderer wrapper.
