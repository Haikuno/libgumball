...
static GblObject* findSiblingContainerWithSelectable_(GblObject* pContainer, bool next) {
    ...
    while (sibling) {
        ...
        if (pButtonObj && GUM_BUTTON(pButtonObj)->isSelectable && GUM_WIDGET(pButtonObj)->shouldUpdate) return sibling;
...
static GUM_Button* findSelectableDescendant(GblObject* pSelf, bool filterByDefault) {
...
static GBL_RESULT GUM_Controller_handleButton_(GUM_Controller* pSelf, GUM_CONTROLLER_BUTTON_STATE eventState,
    ...
    if (isDirectionPress) {
        ...
        if (!(*ppButton)) {
            newButton = findSelectableDescendant(GBL_OBJECT(pRoot), false);
...
static GBL_RESULT GUM_Controller_update_(GUM_Widget* pSelf) {
    ...
    if GBL_UNLIKELY (!GUM_CONTROLLER(pSelf)->pSelectedButton) {
        ...
        GBL_REQUIRE_SCOPE(GUM_Root, &pRoot, "GUM_Root") {
            ...
            GUM_Button* pButton = findSelectableDescendant(GBL_OBJECT(pRoot), true);
