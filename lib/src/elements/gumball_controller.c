...
static GBL_RESULT GUM_Controller_GblObject_setProperty_(GblObject* pObject, const GblProperty* pProp,
                                                        GblVariant* pValue) {
...
static GBL_RESULT GUM_Controller_GblObject_property_(const GblObject* pObject, const GblProperty* pProp,
                                                     GblVariant* pValue) {
    GUM_Controller* pSelf = GUM_CONTROLLER(pObject);
...
static GUM_Button* findSelectableInContainer_(GblObject* pContainer, size_t preferredIndex, bool next) {
    ...
    if (next) {
        GblObject_foreachChild(pContainer, pChild) {
            if (GBL_TYPEOF(pChild) == GUM_BUTTON_TYPE &&
                GUM_BUTTON(pChild)->isSelectable      &&
                GUM_WIDGET(pChild)->shouldUpdate)
                    return GUM_BUTTON(pChild);
        }
    } else {
...
static GUM_Vector2 vector2_FindClosestPointInRec_(GUM_Vector2 startCenter, GUM_Rectangle targetRect) {
    ...
    GUM_Vector2 closestPoint
        = { .x = GBL_CLAMP(startCenter.x, min_x, max_x), .y = GBL_CLAMP(startCenter.y, min_y, max_y) };
...
static GUM_Button* findSelectableByPosition_(GUM_Button* pCurrent, GUM_CONTROLLER_BUTTON_ID direction) {
    ...
    for (size_t i = 0; i < GblArrayList_size(drawQueue); ++i) {
        ...
        if (!pCandidate || pCandidate == pCurrent || !pCandidate->isSelectable
        ...
        GUM_Vector2 candClosestPoint = vector2_FindClosestPointInRec_(
...
static GUM_Button* moveCursor_(GblObject* pSelf, GUM_CONTROLLER_BUTTON_ID buttonPress) {
    ...
    const char grand_parent_orientation = (pGrandParent && GBL_AS(GUM_Container, pGrandParent))
                                              ? tolower(GBL_AS(GUM_Container, pGrandParent)->orientation)
                                              : 'N';
    ...
    size_t preferredIndex  = (new_orientation == parent_orientation && parent_orientation != grand_parent_orientation
...
static GUM_Button* findSelectableDescendant_(GblObject* pSelf, bool filterByDefault) {
    ...
    GblArrayList_construct(&stackArray.array, sizeof(GblObject*), 0, NULL, sizeof(stackArray));

    GblArrayList_pushBack(&stackArray.array, &pSelf);
...
static GBL_RESULT GUM_Controller_handleButton_(GUM_Controller* pSelf, GUM_CONTROLLER_BUTTON_STATE eventState,
    ...
    if (isDirectionPress) {
        ...
        if (!(*ppButton)) {
            newButton = findSelectableDescendant_(GBL_OBJECT(pRoot), false);
...
static GBL_RESULT GUM_Controller_update_(GUM_Widget* pSelf) {
    ...
    if GBL_UNLIKELY (!GUM_CONTROLLER(pSelf)->pSelectedButton) {
        ...
        GBL_REQUIRE_SCOPE(GUM_Root, &pRoot, "GUM_Root") {
            ...
            GUM_Button* pButton = findSelectableDescendant_(GBL_OBJECT(pRoot), true);
...
static GBL_RESULT GUM_Controller_draw_(GUM_Widget* pSelf, GUM_Renderer* pRenderer) {
    ...
    if (pController->pSelectedButton) {
        GUM_Rectangle rec = { .x      = GUM_WIDGET(pController->pSelectedButton)->x,
...
GblType GUM_Controller_type(void) {
    ...
    if (type == GBL_INVALID_TYPE) {
        type = GblType_register(GblQuark_internStatic("GUM_Controller"),
                                GUM_WIDGET_TYPE,
...
GBL_EXPORT void GUM_Controller_sendButton(GUM_Controller* pSelf, GUM_CONTROLLER_BUTTON_STATE state,
